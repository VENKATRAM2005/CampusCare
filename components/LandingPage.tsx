...
const LandingPage: React.FC<LandingPageProps> = ({ onLogin }) => {
  ...
  // Auto-fill logic when user types ID - Restricted to selected role
  useEffect(() => {
    const user = (MOCK_USERS as any)[userId];
    if (user && user.role === selectedRole) {
      setAutoName(user.name);
      setAutoDept(user.dept || '');
    } else {
      setAutoName('');
      setAutoDept('');
    }
      ...
      <div className="max-w-md mx-auto py-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl border border-slate-100">
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-6">
            ...
            {autoName && (
              <div className="p-4 bg-blue-50 border border-blue-100 rounded-2xl text-blue-700 font-bold text-sm flex flex-col animate-in zoom-in-95 duration-200">
                <div className="flex items-center">
                  <Shield className="h-4 w-4 mr-2" />
                  Verified: {autoName}
                </div>
                {autoDept && (
                  <div className="text-[10px] uppercase tracking-widest text-blue-400 mt-1 ml-6">
                    Dept: {autoDept}
                  </div>
                )}
              </div>
    ...
    <div className="max-w-6xl mx-auto py-12">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-lg transition-all text-center flex flex-col">
          ...
          <h2 className="text-lg font-bold mb-1">Mentor</h2>
          <p className="text-slate-500 mb-6 text-sm flex-grow">Maintenance, Administration, or Academic Mentor.</p>
          <button 
            onClick={() => setSelectedRole(Role.MENTOR)}
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-lg transition-all text-center flex flex-col">
          ...
          <p className="text-slate-500 mb-6 text-sm flex-grow">Handles Mentor-Related & Academic escalations.</p>
          <button 
