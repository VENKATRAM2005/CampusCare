...
const Analytics: React.FC<AnalyticsProps> = ({ complaints }) => {
  ...
  const resolvedByRole = [
    { name: 'Mentor', count: complaints.filter(c => c.resolvedByRole === Role.MENTOR).length },
    { name: 'Admin', count: complaints.filter(c => c.resolvedByRole === Role.ADMIN).length },
  ];

  // Escalations over time (Monthly)
  const monthlyEscalations = complaints
    .filter(c => c.escalatedToAdmin)
    .reduce((acc: any, c) => {
  ...
  const escalationTimeData = Object.entries(monthlyEscalations).map(([name, count]) => ({
    name,
    count
  })).sort((a, b) => new Date(a.name).getTime() - new Date(b.name).getTime());

  // Escalations by Category
  const escalationCategoryData = Object.values(ComplaintCategory).map(cat => ({
    name: cat.replace('_', ' '),
    count: complaints.filter(c => c.category === cat && c.escalatedToAdmin).length
  })).filter(d => d.count > 0);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
           <h4 className="text-lg font-black text-slate-800 uppercase tracking-widest mb-8">Escalation Timeline</h4>
           <div className="h-80">
             <ResponsiveContainer width="100%" height="100%">
               <LineChart data={escalationTimeData}>
                 <XAxis dataKey="name" fontSize={10} axisLine={false} tickLine={false} />
                 <YAxis fontSize={10} axisLine={false} tickLine={false} />
                 <Tooltip />
                 <Line type="monotone" dataKey="count" stroke="#EF4444" strokeWidth={3} dot={{ r: 6, fill: '#EF4444' }} />
               </LineChart>
             </ResponsiveContainer>
           </div>
        </div>
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
           <h4 className="text-lg font-black text-slate-800 uppercase tracking-widest mb-8">Escalations by Category</h4>
           <div className="h-80">
             <ResponsiveContainer width="100%" height="100%">
               <BarChart data={escalationCategoryData}>
                 <XAxis dataKey="name" fontSize={10} axisLine={false} tickLine={false} />
                 <YAxis fontSize={10} axisLine={false} tickLine={false} />
                 <Tooltip />
